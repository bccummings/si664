...
class CatCreateView(LoginRequiredMixin, CreateView):
    """
    Sub-class of the CreateView to catmatically pass the Request to the Form
...
class CatUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_queryset(self):
        ...
        qs = super(CatUpdateView, self).get_queryset()
...
class CatDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get_queryset(self):
        ...
        qs = super(CatDeleteView, self).get_queryset()
        return qs.filter(owner=self.request.user)
