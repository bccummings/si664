...
class HorseCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        object.save()
        return super(HorseCreateView, self).form_valid(form)
...
class HorseUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_queryset(self):
        ...
        qs = super(HorseUpdateView, self).get_queryset()
        return qs.filter(owner=self.request.user)

class HorseDeleteView(LoginRequiredMixin, DeleteView):
    """
    ...
    def get_queryset(self):
        ...
        qs = super(HorseDeleteView, self).get_queryset()
        return qs.filter(owner=self.request.user)
